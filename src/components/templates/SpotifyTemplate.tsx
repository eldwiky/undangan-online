...
export default function SpotifyTemplate({ invitation, guestName }: SpotifyTemplateProps) {
  ...
  useEffect(() => {
    if (invitation.musicUrl && isOpened) {
      ...
      // Autoplay after user clicks "Buka Undangan"
      audio.play().then(() => {
        setIsPlaying(true);
      }).catch(() => {
        setIsPlaying(false);
  ...
  const scrollToSection = (id: string) => {
    const el = document.getElementById(`spotify-${id}`);
      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            const id = entry.target.id.replace("spotify-", "");
            setActiveSection(id);
          }
        });
      },
  ...
  if (!isOpened) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-[#121212] px-4 text-center">
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
        ...
        >
          <motion.div
            ...
            className="text-[#1DB954] text-2xl mb-4"
          >
            ♫
          </motion.div>
          ...
          <motion.p
            ...
            className="text-xl text-[#1DB954] mb-2"
          >
            &
          </motion.p>
          ...
          {invitation.hashtag && (
            <motion.p
              ...
              className="text-[#1DB954] text-sm mb-4"
            >
              {invitation.hashtag}
            </motion.p>
          )}
          ...
          {!guestName && !invitation.hashtag && <div className="mb-8" />}
  ...
  return (
    <div className="flex h-screen bg-[#121212] text-white overflow-hidden">
      ...
      <aside className="hidden lg:flex flex-col w-[220px] bg-black p-4 overflow-y-auto shrink-0">
        ...
        <div className="mb-6 px-2">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-full bg-[#1DB954] flex items-center justify-center text-xs font-bold text-black">
              ♫
            </div>
            ...
            <button
              ...
              className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-md text-sm font-medium transition-all ${
                activeSection === item.id
                  ? "text-white bg-[#282828] border-l-2 border-[#1DB954]"
        ...
        <div className="mt-6 pt-4 border-t border-[#282828]">
          <p className="text-[10px] uppercase tracking-widest text-gray-500 mb-3 px-2">Wedding Playlist</p>
          <div className="px-2">
      ...
      <button
        ...
        className="lg:hidden fixed top-4 left-4 z-50 w-10 h-10 bg-[#282828] rounded-full flex items-center justify-center shadow-lg"
        ...
        {sidebarOpen && (
          <>
            ...
            <motion.aside
              ...
              className="lg:hidden fixed left-0 top-0 bottom-0 w-[260px] bg-black z-50 p-4 overflow-y-auto"
            >
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-2">
                  <div className="w-8 h-8 rounded-full bg-[#1DB954] flex items-center justify-center text-xs font-bold text-black">♫</div>
                  <span className="font-bold text-sm">{invitation.groomName.charAt(0)} & {invitation.brideName.charAt(0)}</span>
                </div>
      ...
      <div ref={mainRef} className="flex-1 overflow-y-auto pb-24">
        ...
        {/* ── HERO SECTION ── */}
        <section id="spotify-home" className="relative min-h-[70vh] flex items-end">
          ...
          >
            <p className="text-sm text-gray-300 mb-2 uppercase tracking-widest">The Wedding of</p>
        ...
        {(invitation.quoteText || invitation.quoteArabic) && (
          <section id="spotify-ayat" className="py-16 px-6 md:px-12">
            ...
            >
              <div className="bg-[#1a1a1a] rounded-xl p-8 border border-[#282828]">
                {invitation.quoteArabic && (
                  <p dir="rtl" className="text-2xl md:text-3xl text-gray-100 leading-loose mb-6 font-serif">
                    {invitation.quoteArabic}
                  </p>
                )}
                {invitation.quoteText && (
                  <p className="text-lg md:text-xl text-gray-200 leading-relaxed font-serif">
                    &ldquo;{invitation.quoteText}&rdquo;
                  </p>
                )}
                {invitation.quoteSource && (
                  <p className="mt-4 text-sm text-[#1DB954] font-medium">— {invitation.quoteSource}</p>
                )}
              </div>
            </motion.div>
          </section>
        ...
        {!invitation.quoteText && !invitation.quoteArabic && <div id="spotify-ayat" />}

        {/* ── MEMPELAI ── */}
        <section id="spotify-mempelai" className="py-16 px-6 md:px-12">
          <motion.div
            variants={stagger}
            initial="hidden"
            whileInView="visible"
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {/* Groom */}
              <motion.div variants={fadeInUp} className="bg-[#1a1a1a] rounded-xl p-6 text-center border border-[#282828] hover:border-[#1DB954]/30 transition-colors">
                {invitation.groomPhoto ? (
                  <div className="w-32 h-32 mx-auto mb-4 rounded-full overflow-hidden border-2 border-[#1DB954]/50">
                    <img src={invitation.groomPhoto} alt={invitation.groomName} className="w-full h-full object-cover" />
                  </div>
                ) : (
                  <div className="w-32 h-32 mx-auto mb-4 rounded-full bg-[#282828] flex items-center justify-center">
                    <span className="text-3xl">🤵</span>
                  </div>
                )}
                <h3 className="text-xl font-bold">{invitation.groomFullName || invitation.groomName}</h3>
                {invitation.groomChildOrder && (
                  <p className="text-sm text-gray-400 mt-1 italic">{invitation.groomChildOrder}</p>
                )}
              ...
              <motion.div variants={fadeInUp} className="bg-[#1a1a1a] rounded-xl p-6 text-center border border-[#282828] hover:border-[#1DB954]/30 transition-colors">
                ...
                <h3 className="text-xl font-bold">{invitation.brideFullName || invitation.brideName}</h3>
                {invitation.brideChildOrder && (
                  <p className="text-sm text-gray-400 mt-1 italic">{invitation.brideChildOrder}</p>
                )}
        ...
        {/* ── ACARA ── */}
        <section id="spotify-acara" className="py-16 px-6 md:px-12 bg-[#0a0a0a]">
          <motion.div
            variants={stagger}
            initial="hidden"
            whileInView="visible"
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {(invitation.akadDate || invitation.akadTime) && (
                <motion.div variants={fadeInUp} className="bg-[#1a1a1a] rounded-xl p-6 border border-[#282828]">
                  ...
                  {invitation.akadTime && <p className="text-gray-400 text-sm mt-1">Pukul {invitation.akadTime}</p>}
                  {invitation.akadLocationName && <p className="text-white font-medium mt-3">{invitation.akadLocationName}</p>}
              ...
              {(invitation.resepsiDate || invitation.resepsiTime) && (
                <motion.div variants={fadeInUp} className="bg-[#1a1a1a] rounded-xl p-6 border border-[#282828]">
                  ...
                  {invitation.resepsiTime && <p className="text-gray-400 text-sm mt-1">Pukul {invitation.resepsiTime}</p>}
                  {invitation.resepsiLocationName && <p className="text-white font-medium mt-3">{invitation.resepsiLocationName}</p>}
            ...
            {!invitation.akadDate && !invitation.resepsiDate && (
              <motion.div variants={fadeInUp} className="bg-[#1a1a1a] rounded-xl p-6 border border-[#282828] max-w-md mx-auto text-center">
                ...
                {invitation.eventTime && <p className="text-gray-400 text-sm mt-1">Pukul {invitation.eventTime}</p>}
        ...
        {invitation.loveStories.length > 0 && (
          <section id="spotify-story" className="py-16 px-6 md:px-12">
            <motion.div
              variants={stagger}
              initial="hidden"
              whileInView="visible"
              ...
              <div className="relative">
                ...
                {invitation.loveStories.map((story, index) => (
                  <motion.div key={story.id} variants={fadeInUp} className="relative pl-12 mb-8">
                    <div className="absolute left-2.5 top-2 w-4 h-4 rounded-full bg-[#1DB954] border-2 border-[#121212]" />
                    <div className="bg-[#1a1a1a] rounded-xl p-5 border border-[#282828] hover:border-[#1DB954]/30 transition-colors">
                      {story.date && <p className="text-xs text-[#1DB954] font-medium mb-1">{story.date}</p>}
                      <h4 className="font-bold text-white">{story.title}</h4>
                      <p className="text-sm text-gray-400 mt-2 leading-relaxed">{story.description}</p>
                    </div>
                  </motion.div>
                ))}
              </div>
        ...
        {invitation.loveStories.length === 0 && <div id="spotify-story" />}

        {/* ── GALERI ── */}
        <section id="spotify-galeri" className="py-16 px-6 md:px-12 bg-[#0a0a0a]">
          <motion.div
            variants={stagger}
            initial="hidden"
            whileInView="visible"
        ...
        {/* ── HADIAH / GIFT ── */}
        <section id="spotify-hadiah" className="py-16 px-6 md:px-12">
          <motion.div
            variants={stagger}
            initial="hidden"
            whileInView="visible"
        ...
        {/* ── RSVP / COMMENTS ── */}
        <section id="spotify-rsvp" className="py-16 px-6 md:px-12 bg-[#0a0a0a]">
          <motion.div
            variants={stagger}
            initial="hidden"
            whileInView="visible"
          ...
          >
            ...
            <motion.form
              ...
              className="bg-[#1a1a1a] rounded-xl p-6 border border-[#282828] mb-8"
            >
              <div className="space-y-4">
                <div>
              ...
              {comments.length > 0 ? (
                comments.map((comment, idx) => (
                  <div
                    ...
                    className="bg-[#1a1a1a] rounded-xl p-4 border border-[#282828]"
                  >
                    <div className="flex items-center justify-between mb-2">
                      <span className="font-medium text-white text-sm">{comment.guestName || "Anonim"}</span>
      ...
      {invitation.musicUrl && (
        <div className="fixed bottom-0 left-0 right-0 z-40 bg-[#181818] border-t border-[#282828] px-4 py-2">
          <div className="flex items-center justify-between max-w-screen-2xl mx-auto gap-4">
            ...
            <div className="flex items-center gap-3 min-w-0 w-1/4">
              ...
              <div className="min-w-0">
                <p className="text-sm text-white truncate font-medium">Wedding Song</p>
                <p className="text-xs text-gray-400 truncate">{invitation.groomName} & {invitation.brideName}</p>
              </div>
            </div>
            ...
            <div className="flex flex-col items-center flex-1 max-w-[600px]">
              ...
              <div className="flex items-center gap-2 w-full">
                ...
                <input
                  ...
                  value={currentTime}
                  onChange={seekTo}
                  className="flex-1 h-1 accent-[#1DB954] bg-[#4d4d4d] rounded-full appearance-none cursor-pointer [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-3 [&::-webkit-slider-thumb]:h-3 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-white"
                />
            ...
            <div className="hidden md:flex items-center gap-2 w-1/4 justify-end">
              ...
              <input
                ...
                className="w-24 h-1 accent-[#1DB954] bg-[#4d4d4d] rounded-full appearance-none cursor-pointer [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-3 [&::-webkit-slider-thumb]:h-3 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-white"
              />
            </div>
          </div>
        </div>
